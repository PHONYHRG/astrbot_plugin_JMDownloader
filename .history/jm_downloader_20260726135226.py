...
class JmDownloader:
    ...
    def _images_to_pdf(self, image_dir: Path, output_path: Path) -> Path:
        """将图片文件夹合并为 PDF"""
        try:
    ...
    def download_and_pdf(self, album_id: str, temp_dir: Path) -> Tuple[Optional[Path], Optional[Path]]:
        """
        下载本子并合成 PDF
        ...
        try:
            ...
            self._images_to_pdf(album_dir, pdf_path)
